...
def desafio():
    ...
    while True:
        agregar = input('Ingrese el producto que desea agregar a su stock o ingrese "fin" para finalizar: ')

        if agregar.lower() == "fin":
            print("Ingresos de stock: finalizando")
            break
        if agregar in stock:
            cuanto_sumar = input('Ingrese cuántas unidades desea agregar: ')
            try:
                cuanto_sumar = int(cuanto_sumar)
            except ValueError:
                print("Ingrese un número entero válido.")
                continue
            stock[agregar] += cuanto_sumar
        
        else: 
        
            print(f'el Item {agregar} no existe en su')

    print("Diccionario final: ", stock)
